...
export default function GeoDropsCard({
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const param =
        pathname === "/dashboard/my-geo-drops" &&
        award.public.toString() === "0"
          ? "?my-geo-drops=true"
          : "";

      router.push(
        `/dashboard/award-details/${award.awardid}${mantlepiecesuffix}${param}`
      );
    } catch (error) {
  ...
  const handleEdit = async () => {
    if (!award?.awardid) {
      console.error("No award ID available for editing");
      toast.error("No award ID available for editing");
      return;
    }
    ...
    try {
      ...
      router.push(`/dashboard/edit-award/${award.awardid}${param}`);
    } catch (error) {
      console.error("Error during edit navigation:", error);
  ...
  const getClaimButtonState = () => {
    ...
    if (!notClaimed) return { text: "Check In / View Drop", disabled: false };
    return { text: "Check In / Get Drop", disabled: false };
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1.5 justify-between w-full">
              <div className="flex items-center gap-1.5">
                <Button
                  size="sm"
                  // className="bg-clgeodrops hover:opacity-60 duration-200 ease-in-out text-white rounded-full disabled:opacity-50 disabled:cursor-not-allowed"
                  className="geo-claim-button" 
                  onClick={handleClaim}
