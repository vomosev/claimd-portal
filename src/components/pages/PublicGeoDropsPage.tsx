    ...
    async (awardId: string) => {
      try {
        ...
        if (!currentUsername) {
          ...
          router.push(`/award-details/${awardId}/public`);
          return;
        }
        ...
        router.push(`/award-details/${awardId}/public ? '?mantlepiece=true' : ''}`);
      ...
      <div className="mt-20">
        ...
        <div className="mt-6 grid md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {awards.slice(0, 10).map((award, i) => (
            <PublicGeoDropCard key={award.awardid} award={award} />
          ))}
        </div>
      </div>
    </div>
...
export default PublicGeoDropsPage;
