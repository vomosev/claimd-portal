// src/components/pages/MantlePiecePage.tsx
"use client";
...
type ViewMode = "created" | "claimed";

const MantlePiecePage = () => {
      ...
      try {
        ...
        const [claimedRes, createdRes] = await Promise.all([
          axios.get<claimedAwards[]>(
            `${process.env.NEXT_PUBLIC_API_URL}/claimedawards/listbyuser/${currentUsername}`
          ),
          axios.get<claimedAwards[]>(
            `${process.env.NEXT_PUBLIC_API_URL}/awards/listbyuser/${currentUsername}`
          ),
        ]);
      ...
      {adminStatus && !hasAnyAwards ? (
        <div className="w-full flex flex-col items-center justify-center gap-3 py-60">
          <h3 className="text-2xl font-semibold text-gray-600 dark:text-gray-400">
            You haven't created or unlocked any Drops yet
          </h3>
          <Button
          ...
          >
            {viewMode === "created" ? "Create a Drop now" : "Unlock a Drop now"}
          </Button>
        </div>
      ) : adminStatus && currentAwards.length === 0 ? (
        <div className="w-full flex flex-col items-center justify-center gap-3 py-40">
          <h3 className="text-xl font-semibold text-gray-600 dark:text-gray-400">
            You haven't {viewMode === "claimed" ? "unlocked" : "created"} any Drops yet
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-500 mb-2">
            {viewMode === "claimed" 
              ? "Start exploring and unlock your first Drop!" 
              : "Create your first Drop and share it with the world!"}
          </p>
          <Button
            className="rounded-full disabled:opacity-50 disabled:cursor-not-allowed 
               bg-clgeodrops hover:opacity-60 duration-200 ease-in-out text-white"
            onClick={() => {
              router.push(`/dashboard/add-award`);
            }}
          >
            {viewMode === "created" ? "Create a Drop now" : "Unlock a Drop now"}
          </Button>
...
export default MantlePiecePage;
