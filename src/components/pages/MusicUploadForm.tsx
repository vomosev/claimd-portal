...
// ──────────────────────────────────────────────
// Track Meta Card - used for both single and album
// ──────────────────────────────────────────────
const TrackMetaCard = ({
  index,
  file,
  ...
  return (
    <div className='rounded-lg border border-gray-200 dark:border-[#2D385B] overflow-hidden'>
      ...
      {/* ── Coloured header bar ── */}
      <div className='flex items-center gap-3 px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border-b border-gray-200 dark:border-[#2D385B]'>
      ...
      {/* ── Body ── */}
      <div className='p-4 bg-gray-50 dark:bg-[#1A2235] space-y-3'>

        {/* ── Track title - always visible for albums ── */}
        {showTitle && (
          <div className='p-3 bg-white dark:bg-[#0F1628] rounded-lg border border-blue-200 dark:border-blue-800'>
            <label className='block text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wide mb-2'>
            ...
            <input
              ...
              className='w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-[#2D385B]
...
const TrackMetaCard_old = ({
  ...
  return (
    <div className='p-4 bg-gray-50 dark:bg-[#1A2235] rounded-lg border border-gray-200 dark:border-[#2D385B]'>
      ...
      {/* Track title - only shown for albums */}
      {showTitle && (
        <div className='mb-3'>
...
const MusicUploadForm = () => {
  ...
  // ── trackMeta used for both singles and albums ──
  ...
  // ── Audio file handling ──
  // FIX: always add trackMeta regardless of release type
  const handleAudioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // ── Always add trackMeta for both singles and albums ──
  ...
  // FIX: always remove trackMeta regardless of release type
  const removeAudioFile = (index: number) => {
    updateField('audioFiles', formData.audioFiles.filter((_, i) => i !== index));
    // ── Always remove trackMeta for both singles and albums ──
  ...
  const validate = (): boolean => {
    ...
    if (!formData.releaseDate) {
      ...
      if (chosenDate < minDate)
        newErrors.releaseDate = 'Release date must be at least 5 days from today';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.append('emailAddress',   currentUsername);

      // ── Always send trackMeta for both singles and albums ──
      ...
      if (response.data.success) {
        ...
        router.push('/dashboard/list-music');
      } else {
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
            ...
            <div className='space-y-6'>
              ...
              <div className='bg-white dark:bg-[#151E3A] dark:border dark:border-[#2D385B] rounded-xl shadow-sm p-6'>
                ...
                {/* ── Single: TrackMetaCard without title field ── */}
                ...
                {/* ── Album: TrackMetaCard with title field ── */}
