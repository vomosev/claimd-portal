...
function groupUrls(rows: UrlRecord[]): GroupedUrlRecord[] {
  ...
  for (const row of rows) {
    ...
    if (!map.has(key)) {
      map.set(key, { groupId: row.id, worldid: row.worldid, urls: [], linkname: row.linkname });
    }
...
function bkgUrl(linkname?: string) {
  if (!linkname) return `https://nodejs.gridiron-app.com/images/bkg_${process.env.NEXT_PUBLIC_WORLDID}.png`;
  return `https://nodejs.gridiron-app.com/images/linkimg_${linkname}.png`;
}

// ── Main public page ───────────────────────────────────────────────────────────
export default function PublicLinksPage({ linkName }: PublicLinksPageProps) {
  const params  = useParams();
  const profile = linkName ?? (params?.linkname as string) ?? "";
    ...
    const fetchUrls = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/linkurls?linkname=${encodeURIComponent(profile)}`
        );
  ...
  // ── Derived background ───────────────────────────────────────────────────────
  const primaryBg   = bkgUrl(groups[0]?.linkname);
      ...
      <div className="relative z-10 w-full max-w-md mx-auto px-4 py-16 flex flex-col items-center gap-6">
        ...
        <div className="relative">
          ...
          <div className="relative w-56 h-56 rounded-full border-4 border-white/30 overflow-hidden shadow-2xl bg-white/10 backdrop-blur-md flex items-center justify-center">
            {resolvedBg ? (
              <img
                src={resolvedBg || process.env.NEXT_PUBLIC_LOGO_PATH}
                alt={profile}
        ...
        <p className="text-white/25 text-sm mt-4 tracking-widest">
          {process.env.NEXT_PUBLIC_DNSPREFIX}/geo-drops.com/links/{linkName}
        </p>
