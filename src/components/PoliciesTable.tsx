...
export default function PoliciesTable() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/getuserrole/${currentUsername}`)
      ...
      .then((data) => {
        ...
        if (Number(process.env.NEXT_PUBLIC_INSURANCE) === 1 && isAdmin) {
  ...
  const loadPolicies = useCallback(async (isRefresh = false) => {
    ...
    try {
      const username = localStorage.getItem("username") ?? "";
      console.log(`running ${process.env.NEXT_PUBLIC_API_URL}/ins_policy/list/${username}`);
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/ins_policy/list/${username}`
      );
