// components/WorldUserIdList.tsx
"use client";
...
interface WorldUserIdListProps {
  worldId:           string;
...
// ── Component ──────────────────────────────────────────────────────────────────
export default function WorldUserIdList({
  worldId,
  selectedRecipient,
  onSelectRecipient,
  search,
  ...
  const fetchUsers = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/worldregistrants/${encodeURIComponent(id)}`
      );
      ...
      console.error("Error fetching world registrants:", err);
        ...
        {!worldId && (
          <div className="flex flex-col items-center justify-center py-10 gap-2 text-center">
            ...
            <p className="text-sm text-gray-400">
              Select a Dropsite above to see its registered users.
            </p>
          </div>
        )}
