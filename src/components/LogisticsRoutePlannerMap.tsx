...
export default function LogisticsRoutePlanner({
  ...
  // ── Vehicle filtering ─────────────────────────────────────────────
  const [selectedVehicleId, setSelectedVehicleId] = useState<string | number | "all">("all");

  // ── ETA path refs ────────────────────────────────────────────────
  const etaLinesRef = useRef<Map<string | number, any>>(new Map());
  ...
  const buildEtaPath = (vehicle: any, stops: Stop[]) => {
    if (!vehicle || stops.length === 0) return [];
    ...
    // build forward path: vehicle → remaining stops
    const path = [
      vehiclePos,
      ...stops.slice(closestIndex).map(s => ({
  ...
  const visibleVehicles = selectedVehicleId === "all"
    ? vehicles
  ...
  visibleVehicles.forEach((v) => {
    ...
    // ── ETA PATH ─────────────────────────────────────────────
    const etaPath = buildEtaPath(v, stops);

    if (etaPath.length > 1) {
      const existingLine = etaLinesRef.current.get(id);

      if (existingLine) {
        existingLine.setPath(etaPath);
      } else {
        const line = new window.google.maps.Polyline({
          path: etaPath,
          geodesic: true,
          strokeColor: "#10B981",   // green ETA line
          strokeOpacity: 0.9,
          strokeWeight: 4,
          icons: [
            {
  ...
  }, [stops, route, createInfoContent, vehicles, selectedVehicleId]);
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedVehicleId(val === "all" ? "all" : val);
            }}
            className="text-xs px-3 py-2 rounded-full border border-gray-200 dark:border-[#2E4066] bg-white dark:bg-[#151E3A]"
          >
