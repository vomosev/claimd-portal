// components/EmailWorldBroadcastForm.tsx
"use client";
...
import {
  emailWorldBroadcastApi,
  EmailBroadcast,
} from "@/services/emailWorldBroadcastApi";
import { emailBroadcastApi } from "@/services/emailBroadcastApi";
import WorldUserIdList from "./WorldUserIdList";
...
// ── Component ──────────────────────────────────────────────────────────────────
export default function EmailWorldBroadcastForm({
  mode,
  broadcastId,
      ...
      const fetchBroadcast = async () => {
        try {
          ...
          if (data) {
            ...
            router.push("/dashboard/emailworld-broadcasts");
          }
        } catch {
  ...
  const ensureDraftId = async (values: FormValues): Promise<number | null> => {
    ...
    const draftResult = await emailWorldBroadcastApi.saveDraft({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const result = await emailWorldBroadcastApi.saveDraft(payload);
      if (result.success) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const result = await emailWorldBroadcastApi.sendEmail({
        id,
        userid:  currentUsername,
        worldid: values.worldid,
        subject: values.subject,
      ...
      <div className="lg:w-[85%] space-y-6">
        <div className="flex items-center gap-3">
          <Button type="button" variant="outline" size="sm"
            onClick={() => router.push("/dashboard/emailworld-broadcasts")}
          >
    ...
    <div className="lg:w-[85%] space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-semibold">
            {mode === "new" ? "New Dropsite Broadcast" : "Edit Dropsite Broadcast"}
          </h1>
        </div>
        {currentId && (
      ...
      <Form {...form}>
        <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
          ...
          {/* ── Section 1: Dropsite Selection ─────────────────────────────── */}
          <section className="space-y-6">
            <h2 className="text-xl font-semibold flex items-center gap-2.5">
              <Globe className="text-geodrops" /> Dropsite Selection
            </h2>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Select Dropsite</FormLabel>
                  <Select
                    onValueChange={(value) => {
                  ...
                  <FormDescription>
                    Select a Dropsite. The email will be sent to all users
                    registered to the selected Dropsite.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
          ...
          <section className="space-y-4">
            ...
            <p className="text-sm text-gray-500">
              All users registered to this Dropsite are listed below.
              {allUsers.length > 0 && (
