...
export default function LogisticsFleetMap() {
  ...
  const [markers, setMarkers] = useState<Map<string | number, any>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    if (!navigator.geolocation) {
      console.error("[FleetMap] Geolocation unsupported");
      return;
    }
    ...
    const watchId = navigator.geolocation.watchPosition(
      ...
      (err) => {
        console.error("[FleetMap] GPS watch error:", err);
      },
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      const list: Vehicle[] = raw
        ...
        .filter(
          (v) =>
            !isNaN(v.lat as number) &&
            !isNaN(v.lng as number)
        );
  ...
  const createInfoWindowContent = (v: Vehicle) => {
    ...
    return `
      <div style="font-family:sans-serif; max-width:220px; line-height:1.5;">
        ...
        ${
          v.driver
            ? `<p style="margin:2px 0; font-size:13px;"><strong>Driver:</strong> ${v.driver}</p>`
            : ""
        }

        ${
          v.speed != null
            ? `<p style="margin:2px 0; font-size:13px;"><strong>Speed:</strong> ${Number(v.speed)} km/h</p>`
            : ""
        }

        ${
          v.status
            ? `<p style="margin:2px 0; font-size:13px; text-transform:capitalize;"><strong>Status:</strong> ${v.status}</p>`
            : ""
        }

        ${
          v.last_seen
            ? `<p style="margin:2px 0; font-size:12px; color:#6b7280;"><strong>Last seen:</strong> ${new Date(v.last_seen).toLocaleTimeString()}</p>`
            : ""
        }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        console.warn(
          "[FleetMap] Could not get initial GPS position"
        );
      }
  ...
  const handleGetUserLocation = () => {
    ...
    if (!mapInstance) {
      ...
      console.error("[FleetMap] Map not ready");
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Current location of vehicles and their planned routes.
          </p>
        </div>
      </div>
